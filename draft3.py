...
while True:
    ...
    if cmd == "add":
        # pattern: add A5, B6, C7
        items = [x.strip() for x in args.split(",") if x.strip()]
        
        if not items:
            print("error: no input channels provided")
            continue

        # เก็บจำนวนแขกต่อช่องทาง (dict)
        if not hasattr(hotel, "channel_count"):
            hotel.channel_count = {}

        for it in items:
            ch = ''.join(filter(str.isalpha, it))
            num = ''.join(filter(str.isdigit, it))
            if not ch or not num:
                print(f"error: invalid format '{it}', expected like A5 or B12")
                continue

            ch = ch.upper()
            count = int(num)
            hotel.channel_count[ch] = hotel.channel_count.get(ch, 0) + count

        # รีสร้าง AVL ใหม่ทั้งหมด
        hotel.root = None
        print("\nRebuilding hotel room assignments...")
        for ch, total_guests in hotel.channel_count.items():
