...
# 10/10/2025:
# 1. Changed input pattern for add command to "add 100,200" (numbers of customers for sequential channels starting from next available channel)
# 2. Change Channel from letters (A-Z) to numbers (1,2,3,4,...)
# 3. Modified room ID calculation to (3 ** A) * (2 ** B) where A is channel number and B is customer number (1, 2, ...)
# Note: Each channel can take input only once
class Node:
    ...
    def ID(self,customer_num,channel):
        ...
        if not channel.isdigit():
            return f'error: channel must be a number'
        A = int(channel)
        if A < 1:
            return f'error: channel must be at least 1'
        ...
        B = int(customer_num)
        if not (1 <= B):
            return f'error: customer_num must be at least 1'
...
while True:
    ...
    if cmd == "add":  # new pattern: add 100,200 (numbers of customers for sequential channels starting from next_channel)

        nums = [x.strip() for x in args.split(",")] if args else []
        if not nums:
        ...
        for i, num_str in enumerate(nums):
            ...
            num = int(num_str)
            if not (1 <= num):
                print(f"Error: {num} must be at least 1")
                continue
            ch = str(next_channel + i)
            for customer in range(1, num + 1):
                new_node = Node(str(customer), ch)
                ...
                if hotel.find_room(new_node.roomID) is not None:
                    ...
                    print(f"Added customer: Channel {ch}, Num {customer}, RoomID {new_node.roomID}")
        # Update next_channel
        next_channel += len(nums)
...
"""
input pattern : add 100,200 (numbers of customers for sequential numeric channels starting from next available)
